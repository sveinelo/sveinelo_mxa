...
public class CorrespondenceBuilder {
	 ...
	 * @return an {@link InsertCorrespondenceV2} object on the correspondence
	 *         format
	 * @throws CorrespondenceBuilderException
	 */
	public InsertCorrespondenceV2 buildInsertCorrespondenceV2FromMessageDTO(
			MessageDTO message) throws CorrespondenceBuilderException {
		...
		/**
		 * Correspondences.Correspondence.Notifications.Notification.
		 * ReceiverEndPoints.ReceiverEndPoint
		 */
		...
		insertCorrespondenceV2
				.setAllowSystemDeleteDateTime(getAllowSystemDeleteDateTime());
		insertCorrespondenceV2.setDueDateTime(getDueDateTime());
		insertCorrespondenceV2
				.setArchiveReference(getArchiveReference(message));
		...
		if (message.getAttachments() != null
			...
			for (AttachmentDTO attachmentDTO : message.getAttachments()) {
				...
				binaryAttachmentV2
						.setFileName(getBinaryAttachmentFileName(attachmentDTO));
				binaryAttachmentV2
						.setName(getBinaryAttachmentName(attachmentDTO));
				binaryAttachmentV2
						.setData(getBinaryAttachmentData(attachmentDTO));

				/** Add attachment in list */
				binaryAttachmentExternalBEV2List.getBinaryAttachmentV2().add(
						binaryAttachmentV2);
			}
		}
		...
		if (message.getContactInfo() != null
			...
			for (ContactInfoDTO contactInfoDTO : message.getContactInfo()) {
				...
				receiverEndPoint
						.setTransportType(getTransportType(contactInfoDTO));
				receiverEndPoint
						.setReceiverAddress(getReceiverAddress(contactInfoDTO));

				/** Add receiverEndpoint in list */
				receiverEndPointBEList.getReceiverEndPoint().add(
						receiverEndPoint);
			}

			/** Add receiverEndPoint in notification */
			notification
					.setReceiverEndPoints(getReceiverEndPointBEList(receiverEndPointBEList));
		...
		/** Correspondences.Correspondence.Content.Attachments.BinaryAttachments */
		attachmentsV2
				.setBinaryAttachments(getBinaryAttachmentExternalBEV2List(binaryAttachmentExternalBEV2List));

		/** Set attachments on content */
		externalContentV2.setAttachments(getAttachmentsV2(attachmentsV2));
		/** Set content on correspondence */
		insertCorrespondenceV2
				.setContent(getExternalContentV2(externalContentV2));
		/** Set notifications on correspondence */
		insertCorrespondenceV2
				.setNotifications(getNotificationBEList(notificationBEList));
	...
	private JAXBElement<String> getServiceCode() {
		return objectFactory
				.createInsertCorrespondenceV2ServiceCode(messageValues
						.getServiceCode());
	}
	...
	private JAXBElement<String> getServiceEdition() {
		return objectFactory
				.createInsertCorrespondenceV2ServiceEdition(messageValues
						.getServiceEdition());
	}

	/** Correspondences.Correspondence.Reportee */
	private JAXBElement<String> getReportee(MessageDTO message) {
		return objectFactory.createInsertCorrespondenceV2Reportee(message
				.getParticipantId());
	}
	 ...
	 * 
	 */
	private JAXBElement<XMLGregorianCalendar> getAllowSystemDeleteDateTime()
			throws CorrespondenceBuilderException {
		try {
			XMLGregorianCalendar allowSystemDeleteDateTime = DateUtils.getFutureDate(
					Calendar.YEAR, 5, 0);
			return objectFactory
					.createInsertCorrespondenceV2AllowSystemDeleteDateTime(allowSystemDeleteDateTime);
		} catch (DatatypeConfigurationException e) {
			throw new CorrespondenceBuilderException(
					"Failed to create allowSystemDeleteDateTime", e);
		}
	 ...
	 * @throws CorrespondenceBuilderException
	 */
	private XMLGregorianCalendar getDueDateTime()
			throws CorrespondenceBuilderException {
		try {
			return DateUtils.getFutureDate(Calendar.DAY_OF_MONTH, 7, 0);
		} catch (DatatypeConfigurationException e) {
			throw new CorrespondenceBuilderException(
					"Failed to create dueDateTime", e);
		}
	}
	...
	private JAXBElement<String> getArchiveReference(MessageDTO message) {
		return objectFactory
				.createInsertCorrespondenceV2ArchiveReference(message
						.getAltinnArchive());
	}

	/** Correspondences.Correspondence.Content */
	private JAXBElement<ExternalContentV2> getExternalContentV2(
			ExternalContentV2 externalContentV2) {
		return objectFactory
				.createInsertCorrespondenceV2Content(externalContentV2);
	}
	...
	private JAXBElement<String> getLanguageCode() {
		return objectFactory.createExternalContentV2LanguageCode(messageValues
				.getLanguageCode());
	}
	...
	private JAXBElement<String> getMessageTitle(MessageDTO message) {
		return objectFactory.createExternalContentV2MessageTitle(message
				.getMessageHeader());
	}

	/** Correspondences.Correspondence.Content.MessageBody */
	private JAXBElement<String> getMessageBody(MessageDTO message) {
		return objectFactory.createExternalContentV2MessageBody(message
				.getMessageSummary());
	}

	/** Correspondences.Correspondence.Content.Attachments */
	private JAXBElement<AttachmentsV2> getAttachmentsV2(
	...
	private JAXBElement<BinaryAttachmentExternalBEV2List> getBinaryAttachmentExternalBEV2List(
		...
		return objectFactory
				.createBinaryAttachmentExternalBEV2List(binaryAttachmentExternalBEV2List);
	}
	 ...
	 */
	private JAXBElement<String> getBinaryAttachmentFileName(
			AttachmentDTO attachment) {
		return objectFactory.createBinaryAttachmentFileName(attachment
				.getFileName());
	}

	/**
	 * Correspondences.Correspondence.Content.Attachments.BinaryAttachments.
	 * BinaryAttachment.Name
	 */
	...
	private JAXBElement<byte[]> getBinaryAttachmentData(AttachmentDTO attachment)
		...
		try {
			...
			String attachementAsString = clob.getSubString(1L,
					(int) clob.length());
			binaryAttachmentData = objectFactory
					.createBinaryAttachmentV2Data(attachementAsString
							.getBytes(UniversalConstants.CHARACTER_SET_UTF8));
		} catch (UnsupportedEncodingException e) {
			throw new CorrespondenceBuilderException(
					"Sending av melding feilet. Henting av vedlegg fra databasen feilet",
					e);
	...
	/** Correspondences.Correspondence.Notifications */
	private JAXBElement<NotificationBEList> getNotificationBEList(
			NotificationBEList notificationBEList) {
		return objectFactory
				.createInsertCorrespondenceNotifications(notificationBEList);
	}
	...
	private JAXBElement<String> getNotificationType() {
		return objectFactory.createNotificationNotificationType(messageValues
				.getNotificationType());
	}

	/**
	 * Correspondences.Correspondence.Notifications.Notification.
	 * ReceiverEndPoints.ReceiverEndPoint.TransportType
	 */
	private JAXBElement<TransportType> getTransportType(
	...
	/**
	 * Correspondences.Correspondence.Notifications.Notification.
	 * ReceiverEndPoints.ReceiverEndPoint.ReceiverAddress
	 */
	private JAXBElement<String> getReceiverAddress(ContactInfoDTO contactInfo) {
		return objectFactory.createReceiverEndPointReceiverAddress(contactInfo
				.getAddress());
	}
	 ...
	 * ReceiverEndPoints
	 */
	private JAXBElement<ReceiverEndPointBEList> getReceiverEndPointBEList(
			ReceiverEndPointBEList receiverEndPointBEList) {
		return objectFactory
				.createReceiverEndPointBEList(receiverEndPointBEList);
	}
